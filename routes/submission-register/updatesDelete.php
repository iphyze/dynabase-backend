...
$stmt = dbExecute(
    ...
    'UPDATE submission_register_updates
        SET deleted_at = CURRENT_TIMESTAMP, updated_by = ?, updated_by_id = ?, updated_at = CURRENT_TIMESTAMP
      WHERE id = ? AND submission_id = ? AND deleted_at IS NULL',
    'siii',
    [$actorEmail, $actorId, $updateId, $submissionId]
);
$stmt->close();

writeAuditLog($conn, $authUser, 'submission_register.update_deleted', 'submission_register', $submissionId, [
...
jsonResponse([
    'status' => 'Success',
    'message' => 'Submission progress update removed successfully.',
]);
